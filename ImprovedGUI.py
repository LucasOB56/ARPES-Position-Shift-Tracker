...
tab2_layout = [
    ...
    [sg.Graph(canvas_size=(2 * graphsize, graphsize), graph_bottom_left=(-20, -yaxismax_x - 10),
              graph_top_right=(xaxismax + 10, yaxismax_x + 10), key="-xgraph-", background_color="white",
              tooltip="Graph of x-displacement", float_values=True)],
...
tab3_layout = [
    [sg.Graph(canvas_size=(vidsize, vidsize), graph_bottom_left=(0, 0), graph_top_right=(vidsize, vidsize),
              key='-image-', background_color="black", enable_events=True, motion_events=True, drag_submits=True)],
...
layout = [
    ...
    [sg.Button("Start", key="-start-"), sg.Button("Stop", key="-stop-"), sg.Button('Start Live Video', key="-live-"),
...
def exportcsv(indexes, alg2_x_pixel_offset, alg2_y_pixel_offset, time_images):
    ...
    row_index = 0
    with open('pixel_vs_displ' + datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f") + '.csv', 'w',
              newline='') as csvfile:
...
while lock == False:
    ...
    if nret == True and block != True:
        ...
        if testmode == False:
            ...
            #if testmode is on, this creates random inputs to test as data
            xdisp = random.uniform(-randrange,randrange)
            ydisp = random.uniform(-randrange, randrange)
            #print(xdisp, ydisp)

            alg2_x_pixel_offset.append(xdisp)
            alg2_y_pixel_offset.append(ydisp)

        #checks to see if xdisp and ydisp are larger than the existing maximum value
        if abs(xdisp) > testmax_x:
            testmax_x = abs(xdisp)
            print(testmax_x, 'testmax_x')
        if abs(ydisp) > testmax_y:
            testmax_y = abs(ydisp)
        ...
        if graphtab == True:
            window["-xgraph-"].erase()
            window["-ygraph-"].erase()
            xaxismax = max(index, 25)
            yaxismax_x = round(testmax_x)
            yaxismax_y = round(testmax_y)
            ...
            window["-xgraph-"].change_coordinates(graph_bottom_left=(-5, -yaxismax_x - 2),
                                                graph_top_right=(xaxismax + 2, yaxismax_x + 2))
            window["-ygraph-"].change_coordinates(graph_bottom_left=(-5, -yaxismax_y - 2),
    ...
    #Lets the program know what tab the user is on
    if event == "-graphtab-":
        graphtab = True
        datatab = False
    ...
    if event == "-stop-":
        block = True
        start = True
        if len(indexes) > 0 and testmode == False:
